...
/* 16像素高字模压缩为8像素高，供左右双栏小字界面使用。 */
static uint8_t OLED_Compress16To8(uint8_t TopByte, uint8_t BottomByte)
{
	uint8_t i;
	uint8_t Result = 0;
	...
	for (i = 0; i < 4; i++)
	{
		if (((TopByte >> (i * 2)) & 0x01) || ((TopByte >> (i * 2 + 1)) & 0x01))
		{
			Result |= (uint8_t)(1 << i);
		}
		if (((BottomByte >> (i * 2)) & 0x01) || ((BottomByte >> (i * 2 + 1)) & 0x01))
		{
			Result |= (uint8_t)(1 << (i + 4));
		}
	}
...
void OLED_ShowSmallChar(uint8_t Line, uint8_t Column, char Char)
{
	...
	for (i = 0; i < 8; i++)
	{
		OLED_WriteData(OLED_Compress16To8(
			OLED_F8x16[Char - ' '][i],
			OLED_F8x16[Char - ' '][i + 8]));
	}
}
...
void OLED_ShowSmallChinese(uint8_t Line, uint8_t Column, uint8_t Index)
{
	...
	for (i = 0; i < 8; i++)
	{
		TopByte = OLED_HZ16[Index][i * 2] | OLED_HZ16[Index][i * 2 + 1];
		BottomByte = OLED_HZ16[Index][i * 2 + 16] | OLED_HZ16[Index][i * 2 + 17];
		OLED_WriteData(OLED_Compress16To8(TopByte, BottomByte));
	}
}
	...
	const uint8_t *IndexList, uint8_t Count)
{
	...
	for (i = 0; (i < Count) && (Column + i <= 16); i++)
	{
		OLED_ShowSmallChinese(Line, Column + i, IndexList[i]);
	}
}

/* 16x16汉字：Column按8像素字符列计算，一个汉字占2列。 */
void OLED_ShowChinese(uint8_t Line, uint8_t Column, uint8_t Index)
{
	uint8_t i;

	if ((Line < 1) || (Line > 4) || (Column < 1) || (Column > 15))
	{
		return;
	}
	if (Index >= 50)
	...
	const uint8_t *IndexList, uint8_t Count)
{
	...
	for (i = 0; (i < Count) && (Column <= 15); i++)
	{
		OLED_ShowChinese(Line, Column, IndexList[i]);
		Column += 2;
	}
}
...
void OLED_ShowSmallNum(uint8_t Line, uint8_t Column, uint32_t Number, uint8_t Length)
{
	...
	for (i = 0; i < Length; i++)
	{
		OLED_ShowSmallChar(Line, Column + i,
			(char)('0' + Number / Divisor % 10));
		Divisor /= 10;
	}
}
